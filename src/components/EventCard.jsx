...
export default function EventCard({ event, index }) {
  const isLink = event.link && event.link !== "#";
  const Component = isLink ? motion.a : motion.div;

  return (
    <Component
      {...(isLink ? { href: event.link, target: "_blank", rel: "noopener noreferrer" } : {})}
      style={{ textDecoration: 'none', color: 'inherit', cursor: isLink ? 'pointer' : 'default' }}
